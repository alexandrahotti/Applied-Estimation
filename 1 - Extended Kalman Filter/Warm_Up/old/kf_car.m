...
% P(k+1|k) = A * P * A^T + G * R * G^T  #Så jag antar att G är gaussian white
%                                        noise med I som cov och vi ggr in
%                                        covariansen R
% 
...
% Containers for storing data during simulation
X = zeros(2,n+1);
Xhat = zeros(2,n+1);
PP = zeros(4,n+1);
KK = zeros(2,n);
...
for k = 1:n
    ...
    % Simulate system 
    x = A * x + B * u + [wStdP*randn(1,1); wStdV*randn(1,1)]; % The simulated next state based on our linear model.
    y = C * x + D * vStd*randn(1,1); % The simulated next measurement base on the simulated state (Which is based on  the previous state.
    X(:,k+1) = x;

    % Prediction
    xhat = A * xhat + B * u; % State update after control action u. xhat to the right is the previous state.
    P = A * P * A' + G * R * G'; % Update of the covariance.

    % Measurement update
    K = P * C' * inv(C * P * C' + D * Q * D'); % Kalman gain.
    xhat = xhat + K * (y - C * xhat); %   state update wrt the measurement. The innovation: the diff between the actual measurment y and the expectation C*xhat
    P = P - K * C * P; % Update of the covariance
    Xhat(:,k+1) = xhat;
    KK(:,k) = K;
...
figure(2)
sgtitle('Estimated error covariance')
subplot(2,1,1),
plot(sqrt(PP(1,:)))
title('sqrt(P(1,1))')
subplot(2,1,2),
...
% Plot Kalman Gain
figure(3)
sgtitle('Kalman filter gain coefficients')
subplot(2,1,1),
plot(KK(1,:))
title('K(1)')
subplot(2,1,2),
