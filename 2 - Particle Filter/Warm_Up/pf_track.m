...
params.state_space_dimension = 2; % either 3 or 2
params.Sigma_Q = diag([300 300]); % measurement noise covariance matrix. 0.0001 and 10000) diag([100 100]);
params.M = 1000;
...
switch params.state_space_dimension
    case 2
        params.Sigma_R = diag([0.002 0.002]);%diag([0.02 0.02]); % process noise covariance matrix
       % if params.omega_0; error('2D state space can not use omega_0'); end;
    case 3
	params.omega_0 = 2*pi/688;
        params.Sigma_R = diag([5 5 0.04])*0.8; %diag([2 2 0.01]);
end
dt = 1;
%%
