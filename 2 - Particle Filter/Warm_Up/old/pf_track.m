...
mean_S = zeros(2,K); % The center of the distribution also called xhat.
figure(1);
clf;
%%
...
for i = 1 : K
    ...
    switch RESAMPLE_MODE
        case 0
            S = S_bar;
        case 1 % Default. Ändra en parameter högre upp.
    ...
    if VERBOSE > 1
        plot(Z(1,i),Z(2,i),'rx','MarkerSize',10); % The measurements(red X)
        hold on;
        plot(X(1,i),X(2,i),'go','MarkerSize',20); % The true target movements
%         hold on;
        plot(S.X(1,:),S.X(2,:),'b.'); % The particles
        hold off;
        axis([0 640 0 480]);
...
err_z = sqrt(sum((Z - X).^2,1)); % Difference between measurements Z and the true target movement X.

% At the end of the simulation, the mass center of the particle set
%at each time step is considered to be the estimate of the filter and the estimates
%are compared to the ground truth in: err_xhat
...
if VERBOSE
    figure(3);
    clf;
    plot(err_z,'r-'); % The error in the measurements
    hold on;
    plot(err_xhat,'b-'); % The error in the estimates where we place the state 
                         % in the center. and compare the mean of the particles to the ground truth X.
